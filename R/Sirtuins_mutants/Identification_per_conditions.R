...
my_data <- lapply(my_data_f, function(x) {
    ...
    dplyr::mutate(., Condition = dplyr::case_when(
        Condition == "Pel_WT" ~ names(my_colo)[1],
        Condition == "Pel_dKDAC" ~ names(my_colo)[2],
        Condition == "Pel_dNpdA" ~ names(my_colo)[3],
        Condition == "Pel_dNpdA_dKDAC" ~ names(my_colo)[4]
    )) %>%
    unique(.)
...
my_data_wide <- my_data %>%
    ...
    tidyr::pivot_wider(
        data = ., names_from = Condition, values_from = value, values_fill = FALSE)
...
data.table::fwrite(
    x = my_data_wide,
    file = "C:/Users/nalpanic/SynologyDrive/Work/Colleagues shared work/Brandon_Robin/Abaumannii_mutants/Analysis/Identified_sites/Overlap_detected_sites.txt",
